...
- **PER-OS RELEASE BUNDLES (v0.1.2, 2026-06-12):** release now ships two self-contained, OS-native bundles instead of one zip + loose exe: `v3ke-<ver>-linux-amd64.tar.xz` (contains `v3ke`) and `v3ke-<ver>-windows-amd64.zip` (contains `v3ke.exe`). Same 5 OS-independent device artifacts + docs + licenses + one shared provenance manifest in each (manifest byte-identical across both bundles + the standalone). release.py: `ReleasePlatform`/`RELEASE_PLATFORMS`, `bundle_name`, `write_release_bundles` (tar.xz is byte-deterministic — manual TarInfo fixed mtime/uid/gid/mode, GNU_FORMAT+lzma); compat aliases `write_release_zip`/`release_zip_name` kept (now tar.xz). release.yaml fetches the windows exe into tools/v3ke BEFORE packaging; SHA256SUMS + gh release cover both bundles. v0.1.2 verified: `sha256sum -c` OK for both, windows zip self-contained (13 members), cosign-signed. Suite 484 passed.
- **WINDOWS v3ke.exe SHIPPING (v0.1.1, 2026-06-12):** release.yaml gained a `build-v3ke-windows` job (`runs-on: windows-2025`) that docker-runs Corey's pinned self-contained Windows image `ghcr.io/coreyleavitt/nim@sha256:0d179a90…` (custom nim 2.2.10 + MSVC/VCC, build 26100; presets git safe.directory C:\app) and compiles `nim c --cc:vcc` → clean PE32+ `v3ke.exe` (no MinGW runtime). windows-2025 host build 26100 MUST match the container for process isolation (2022/20348 can't run it). Image pinned by digest, fail-closed. Shipped as a loose cosign-signed per-OS asset `v3ke-0.1.1-windows-amd64.exe` (firmware/host artifacts are OS-independent). Worked first try; v0.1.1 verified (PE32+ x86-64, sha matches signed SHA256SUMS). See [[custom-nim-and-windows-vcc]]. Future polish: per-OS zips instead of a loose exe.
- **v0.1.0 RELEASED (2026-06-09):** first real GitHub Release is live — `gh release view v0.1.0` (non-prerelease, cosign-signed). Assets: `v3ke-0.1.0-linux-amd64.zip` (13 members: firmware/{katapult,klipper}.bin, host/{c_helper.so,klipper.elf,klipper.dict}, v3ke CLI, INSTALL/SOURCES, LICENSES, manifest.json), standalone `manifest.json` (build.id 0.1.0, reproducible=true, 5 artifacts), `SHA256SUMS`, `SHA256SUMS.sig`. Cut via `gh workflow run release.yaml -f bump=minor -f prerelease=no` (VERSION 0.0.0→0.1.0; nopal dispatch-button). VERSION on main is now `0.1.0`.
  - **First-real-release shakedown fixed 4 gaps** (release path had only been hermetically tested): (1) `release.yaml` never built the gitignored **`tools/v3ke/v3ke`** CLI → added a `nimble build -d:release` step in the public nim image; (2) `build.py release` needs **jsonschema** (host-side manifest validation) → added setup-uv + run packaging under `uv run --project tools`; (3) added an explicit `build.py artifacts` step so packaging doesn't depend on repro-check's side effects; (4) `write_release_zip` only embedded manifest.json IN the zip — now also writes a **standalone `dist/manifest.json`** (SHA256SUMS + the release asset list expect it); (5) `gh release create` was passing the **invalid `--fail-if-exists`** flag (not a real flag → usage error → exit 1) — removed. Commits up to `5ca5a9a`/release commit on `main`.
  - **Orphaned-tag recovery (expected, per the dispatch-button design):** each failed attempt's `prepare-version` still created the `v0.1.0` tag + bumped VERSION before the later step failed; recovered each time by deleting the tag (`gh api -X DELETE …/git/refs/tags/v0.1.0`) + force-resetting `main` past the junk commit, then re-triggering. Took 3 attempts (SHA256SUMS → gh release create → success).
- **TOOLCHAIN IMAGE PUBLISHED + FULL PIPELINE GREEN ON CI (2026-06-09):** bootstrapped `ghcr.io/coreyleavitt/v3ke-toolchain` and the **entire** CI pipeline now passes end-to-end (test + detect-image + build+ABI + repro a/b + repro-compare byte-identical). Pinned digest = `sha256:7597f5017279…` in `toolchain/IMAGE_DIGEST`. Milestone: first time B3/B4 (build + reproducibility proof) actually ran on GitHub. Final commits `c1b11c3`→`b969e60` on `origin/main`.
  - **Containerfile rewritten single-stage → MULTI-STAGE (Corey caught the bloat):** old image was **61 GB** (baked crosstool-ng's ~55 GB build scratch into the shipped layers); now **4.07 GB**. Four stages: `toolchain-base` (shared) → `mips-build` + `arm-build` (independent, parallelizable, per-toolchain cache isolation) → `final` (COPY only `/opt/x-tools` from both builders + runtime deps). Validated via `build.py artifacts` (13/13 + ABI PASSED) before publishing.
  - **5 issues fixed to get there:** (1) multi-stage split itself; (2) `final` needs host **`cpp`** (klipper/katapult preprocess their linker scripts with bare host cpp — not target compilation); (3) `final` cross compilers need only glibc from host (companion gmp/mpfr/mpc/isl static — ldd-verified); (4) **`git config --system safe.directory '*'`** in `final` — the orchestrator runs git in the bind-mounted `/work`, which under docker is owned by the runner uid ≠ container-root → dubious-ownership exit 128 (rootless podman remapped uids, so only bit in CI); (5) `test_containerfile_b1` updated for the multi-stage `ARG BASE=…@sha256:` + internal-stage FROMs.
  - **build.py image --push digest bug fixed (`b969e60`):** it captured the digest via `inspect RepoDigests`, which did NOT match the registry manifest (b004… vs real 416c…) → "manifest unknown" pull failures. Now uses podman `push --digestfile` (docker: parse push stdout) via new `extract_pushed_digest`; `parse_repo_digest` removed; fixed a `{{64}}`→`{64}` cosmetic in the IMAGE_DIGEST comment header.
  - **Disk:** reclaimed ~225 GB locally (pruned the old 61 GB fat image + the 55 GB builder scratch; tagged 4 GB lean image kept). NB: a `podman container prune` also cleared *other projects'* stopped/exited containers (intonaco/nim-matrix/amoxtli) — harmless but cross-repo.
  - **Image-rebuild flow for Corey:** `docker login ghcr.io` (or it's cached) → `python3 tools/build.py --runtime podman --image ghcr.io/coreyleavitt/v3ke-toolchain:latest image --push` rebuilds + republishes + re-pins IMAGE_DIGEST in one shot (now digest-correct). Commit the IMAGE_DIGEST change.
...
- **CI WHOLE-RUN GREEN (2026-06-07, commit `7ededdd`):** added a `detect-image` job that reads `toolchain/IMAGE_DIGEST` and outputs `has_image`; `build`/`repro` now `needs: detect-image` + `if: has_image == 'true' && (…)`, so they **SKIP** (not fail) before the bootstrap, and `repro-compare` auto-skips via `needs: repro`. Run conclusion = success (test ✓, detect-image ✓, build/repro/compare skipped). Once Corey runs `build.py image --push` and commits the real digest, `has_image` flips true and they run normally — no further workflow edits. **`release.yaml` deliberately still hard-fails** on the placeholder (a release must never ship without the verified image). So: CI green now; release gated on the one-time ghcr bootstrap (`docker login ghcr.io` + `build.py image --push`, see `docs/toolchain-image.md`).
