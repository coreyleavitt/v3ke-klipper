...
- **A5b WAIVED (intentional, Corey 2026-06-06):** chose **keep** the 5 legacy bash build scripts as a manual fallback (solo user, no CI). The Python build path that replaces them is proven locally (A5a real build + B4 repro), so this is a deliberate choice, not a blocker. Do NOT delete the scripts unless Corey reverses this.
...
|----|-----|---------|--------|----------------|
| —  | —   | (populated at stage 4 /code-review) | — | — |
